...
export default function SlotMachine({ onSpin, isSpinning, disabled = false }: Props) {
    ...
    const handleSpin = async () => {
        ...
        try {
            ...
            const finalReels = result.combination.map(
                (id) => [SYMBOLS.find((s) => s.id === id)?.symbol || SYMBOLS[0].symbol]
            );
            setReels(finalReels);
